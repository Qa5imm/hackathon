...
app.post("/", requireAuth, zValidator("json", createLeaseSchema), async (c) => {
  try {
    ...
    const lenderDetails = await userRepository.findById(
      itemDetails?.userId || "",
    );
    ...
    if (borrowerDetails && itemDetails) {
      if (borrowerDetails.coins < totalAmount) {
        ...
        await userRepository.updateUserCoins(
          borrowerId,
          borrowerDetails.coins - totalAmount,
        );
        await userRepository.updateUserCoins(
          lenderId,
          (lenderDetails?.coins || 0) + totalAmount,
        );
...
app.patch(
  "/:id/status",
  requireAuth,
  zValidator("json", updateLeaseStatusSchema),
  async (c) => {
    const id = c.req.param("id");
    const { status } = c.req.valid("json");
    const lease_found = await leaseRepository.findLeaseById(id);
    const borrowerDetails = await userRepository.findById(
